  ...
  return (
    <View>
      ...
      <View style={styles.controlsRow}>
        <Pressable style={[styles.control, { borderColor: theme.border, backgroundColor: theme.surface }]} onPress={() => seekBy(-10)}>
          <Text style={{ color: theme.text }}>-10s</Text>
        </Pressable>
        <Pressable
          style={[styles.control, styles.mainControl, { borderColor: theme.accent, backgroundColor: theme.accent }]}
          onPress={() => (isPlaying ? player.pause() : player.play())}>
          <Text style={{ color: theme.accentOn, fontWeight: '700' }}>{isPlaying ? 'Pause' : 'Play'}</Text>
        </Pressable>
        <Pressable style={[styles.control, { borderColor: theme.border, backgroundColor: theme.surface }]} onPress={() => seekBy(10)}>
          <Text style={{ color: theme.text }}>+10s</Text>
        </Pressable>
        ...
        {SPEEDS.map((speed) => {
          ...
          return (
            <Pressable
              ...
              <Text style={{ color: active ? theme.accentOn : theme.text, fontWeight: active ? '600' : '400' }}>{speed}x</Text>
            </Pressable>
          );
        })}
      </View>
...
const styles = StyleSheet.create({
  ...
  control: { flex: 1, borderWidth: 1, borderRadius: RADIUS.button, paddingVertical: 12, alignItems: 'center' },
  mainControl: { flex: 1.2 },
  speedRow: { flexDirection: 'row', marginTop: 16, gap: 8, justifyContent: 'center' },
  speedButton: { borderWidth: 1, borderRadius: RADIUS.pill, paddingVertical: 8, paddingHorizontal: 14 },
});
